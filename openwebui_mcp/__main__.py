...
async def update_model(params: ModelUpdateParam, ctx: Context) -> dict[str, Any]:
    ...
    return await get_client().update_model(
        params.model_id, params.name, meta, model_params, get_user_token()
    )
...
async def get_prompt(params: PromptIdParam, ctx: Context) -> dict[str, Any]:
    """Get a specific prompt's full content by its command name.

    Pass the command WITHOUT a leading slash (e.g. 'scout-creative').
    """
...
async def delete_prompt(params: PromptIdParam, ctx: Context) -> dict[str, Any]:
    """Delete a prompt by its command name. Pass command WITHOUT leading slash."""
